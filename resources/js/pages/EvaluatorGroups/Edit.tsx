...
import { type EvaluatorGroup, type Branch, type Department } from '@/types/evaluator-group.d';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: 'Edit Evaluator Group',
        href: null,
    },
];

export default function EditEvaluatorGroup({
    evaluatorGroup,
    questionGroups,
    employees,
    branches,
    ...
    const { data, setData, put, errors, processing } = useForm({
        name: evaluatorGroup.name || '',
        question_group_id: evaluatorGroup.question_group_id?.toString() || '',
        employee_ids: (evaluatorGroup.employees || []).map((e) => e.id) as number[],
    });
    ...
    function submit(e: React.FormEvent<HTMLFormElement>) {
        e.preventDefault();
        put(`/evaluator-groups/${evaluatorGroup.id}`);
    }

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Edit Evaluator Group" />
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <Card>
                    <CardHeader className="flex items-center justify-between">
                        <CardTitle>Edit Evaluator Group</CardTitle>
                        <CardAction>
