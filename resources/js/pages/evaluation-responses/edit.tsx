...
export default function Edit({ evaluationResponse, evaluationPeriods, evaluationTypes, users, questions }: Props) {
    const { data, setData, put, processing, errors, reset } = useForm({
        evaluator_id: evaluationResponse.evaluator_id.toString(),
        evaluate_id: evaluationResponse.evaluate_id.toString(),
        evaluation_period_id: evaluationResponse.evaluation_period_id.toString(),
        evaluation_type_id: evaluationResponse.evaluation_type_id.toString(),
        comment: evaluationResponse.comment || '',
        question_responses: evaluationResponse.question_responses?.map(qr => ({
            question_id: qr.question_id.toString(),
            score: qr.score.toString()
        })) || [] as QuestionResponse[],
    });

    const [selectedEvaluationType, setSelectedEvaluationType] = useState<string>(evaluationResponse.evaluation_type_id.toString());

    useEffect(() => {
        setSelectedEvaluationType(data.evaluation_type_id);
    }, [data.evaluation_type_id]);

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('evaluation-responses.update', evaluationResponse.id), {
            onSuccess: () => {
        ...
        <AppLayout>
            ...
            <div className="space-y-6">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-3xl font-bold">Edit Evaluation Response</h1>
                </div>
